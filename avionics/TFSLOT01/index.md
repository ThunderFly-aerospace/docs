...
[TFSLOT](https://github.com/ThunderFly-aerospace/TFSLOT01) sensor is an airspeed sensor designed to be used on UAVs to measure [IAS](https://en.wikipedia.org/wiki/Indicated_airspeed). Due to the 3D printed case, it is possible to optimize the characteristics according to the location of the sensor on the UAV and specific application requirements on measuring range. The first use of this sensor is on ThunderFly autogyro [TF-G2](/instruments/TF-G2/).

{: .origin }
TFSLOT01 was born out of a recurring failure we encountered with our TF-G1 and [TF-G2](/instruments/TF-G2/) autogyros operating in real-world conditions. Standard pitot tubes kept clogging — mud and snow would pack into the stagnation port during low-altitude flight and landings in bad weather, causing airspeed to read zero at the worst possible moment. Every conventional fix we tried (covers, heating, flushing ports) introduced its own compromises. The root cause was the stagnation point itself, so we eliminated it entirely: TFSLOT01 uses the Venturi effect rather than direct impact pressure, meaning there is no port facing into the airflow for debris to enter. The fix proved very reliable in field conditions, and the sensor became standard equipment on TF-G2. For weight-critical platforms, its design later inspired the lighter [TFPITOT01](/avionics/TFPITOT01/).
